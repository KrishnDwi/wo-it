            ...
            <form method="POST" action="/admin/order/{{ $order->id }}/update" style="display: grid; gap: 1.5rem;">
                ...
                <div class="card">
                        ...
                        <div>
                            ...
                            <div style="padding: 0.75rem; background: #f1f5f9; border-radius: 0.5rem; font-weight: 700; color: #475569;">
                                ...
                                @else
                                    Completed
                                @endif
                                <span style="font-weight: 400; font-size: 0.85rem; display: block; margin-top: 0.25rem;">Change status via the work order detail page.</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div style="margin-top: 1.5rem;">
                        ...
                        <textarea name="description" id="description" rows="4" placeholder="Detail description of the issue..." style="width: 100%; padding: 0.75rem; border: 1px solid #cbd5e1; border-radius: 0.5rem; font-size: 0.95rem; font-family: inherit;">{{ $order->description }}</textarea>
                    </div>
                </div>

                <div style="display: flex; gap: 1rem; flex-wrap: wrap;">
                    <button type="submit" style="background: #10b981; color: white; border: none; padding: 0.85rem 1.5rem; border-radius: 0.75rem; font-weight: 700; cursor: pointer; box-shadow: 0 2px 4px rgba(16, 185, 129, 0.2);">save changes</button>
                    
                    <a href="/admin/order/{{ $order->id }}" style="background: #6b7280; color: white; padding: 0.85rem 1.5rem; border-radius: 0.75rem; text-decoration: none; font-weight: 700;">cancel</a>
                    
                    <button type="button" onclick="confirmDelete()" style="background: #ef4444; color: white; border: none; padding: 0.85rem 1.5rem; border-radius: 0.75rem; font-weight: 700; cursor: pointer; box-shadow: 0 2px 4px rgba(239, 68, 68, 0.2);">delete</button>
                </div>
            </form>
